-- Create returns table for managing order returns (CORRECTED VERSION)
-- This migration adds support for returns functionality with correct data types

-- First, let's check what data types we should use
DO $$
DECLARE
    org_id_type TEXT;
    driver_id_type TEXT;
BEGIN
    -- Check organizations table id type
    SELECT data_type INTO org_id_type
    FROM information_schema.columns 
    WHERE table_name = 'organizations' AND column_name = 'id';
    
    -- Check drivers table id type
    SELECT data_type INTO driver_id_type
    FROM information_schema.columns 
    WHERE table_name = 'drivers' AND column_name = 'id';
    
    RAISE NOTICE 'Organizations id type: %, Drivers id type: %', org_id_type, driver_id_type;
END $$;
